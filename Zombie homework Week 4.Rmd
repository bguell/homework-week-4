...
nrolls <- 99
roll <- function(x) {
    sample_n(d, x, replace = FALSE)
}

sample_thing <- roll(nrolls) 
sample_thing #okay i think this worked! got 99 more samples....

mean_of_sample_variables2<-sample_thing %>%
summarise(mean(height), mean(weight), mean(age), mean(zombies_killed), mean(years_of_education))
mean_of_sample_variables2
